...
def train_fn(
    ...
    SID_used_uniq_layer: bool = False,
    SID_emb_comb_method = "sum",
    ...
    if use_SID:
        embedding_module_type = "SID"
        logging.info(f"Using SID embedding module.")
        
        assert os.path.exists(f"./tmp/processed/{dataset_name}/SID_codebook_{SID_data_pth_sfx}.pkl"), \
            f"SID codebook file ./tmp/processed/{dataset_name}/SID_codebook_{SID_data_pth_sfx}.pkl doesn't exist!"
        assert os.path.exists(f"./tmp/processed/{dataset_name}/SID_lookup_{SID_data_pth_sfx}.pkl"), \
            f"SID lookup file ./tmp/processed/{dataset_name}/SID_lookup_{SID_data_pth_sfx}.pkl doesn't exist!"
    
    ######### binning checking #########
    assert os.path.exists(binning_file), f"Binning file {binning_file} doesn't exist! Must be provided for this monitoring"
    movie_binning = pkl.load(open(binning_file, "rb"))["movie_id_2_bins"]
    mov_id_arr = np.array(list(movie_binning.keys()))
    bin_id_arr = np.array(list(movie_binning.values()))

    num_bins = np.unique(bin_id_arr).shape[0]
    num_items_in_bins = {i: np.sum(bin_id_arr == i) for i in range(num_bins)}
    ...
    dataset = get_reco_dataset(
        ...
        use_SID=use_SID,
        SID_data_pth_sfx=SID_data_pth_sfx
    )
    ...
    elif embedding_module_type == "SID":
        embedding_module: EmbeddingModule = LocalSIDEmbeddingModule(
            emb_tbl_size = SID_emb_tbl_size,
            num_items=dataset.max_item_id,
            item_embedding_dim = item_embedding_dim,
            codebook = dataset.SID_codebook,
            lookup = dataset.SID_lookup,
            emb_method = SID_emb_method,
            emb_comb_method = SID_emb_comb_method,
            scale_SID_emb = SID_scale_SID_emb, 
            used_uniq_layer = SID_used_uniq_layer,
            use_num_codebook_layers = SID_use_num_codebook_layers,
            # use_sparse = False,
        )
    else:
    ...
    if sampling_strategy == "in-batch":
        ...
        negatives_sampler = LocalNegativesSampler(
            num_items=dataset.max_item_id,
            local_emb_module=model._embedding_module,
    ...
    # also send bins to device
    easy_index_bins = easy_index_bins.to(device)
    ...
    for epoch in range(num_epochs):
        ...
        for row in iter(train_data_loader):
            ...
            if sampling_strategy == "in-batch":
                ...
                #  `_item_emb`.
                negatives_sampler.local_emb_module = model.module._embedding_module
        ...
        # eval per epoch
        eval_dict_all = [None for _ in range(num_bins)]
        ...
        for eval_iter, row in enumerate(iter(eval_data_loader)):
            ...
            for bi in range(num_bins):
                ...
                if eval_dict_all[bi] is None:
                    eval_dict_all[bi] = {}
                    for k, v in eval_dict.items():
                        eval_dict_all[bi][k] = []

                for k, v in eval_dict.items():
                    eval_dict_all[bi][k] = eval_dict_all[bi][k] + [v]
                del eval_dict
        ...
        for bi in range(num_bins):
            ...
            add_to_summary_writer(
                writer,
                batch_id=epoch,
                metrics=eval_dict_all[bi],
                prefix=f"eval_epoch_bin{bi}",
                world_size=world_size,
            )
...
class DDP_dummy(torch.nn.Module):
    # in case wants to use sparse
    def __init__(self, hstu_model):
        super(DDP_dummy, self).__init__()
        self.module = hstu_model
